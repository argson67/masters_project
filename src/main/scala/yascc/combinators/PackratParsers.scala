    ...
    head match {
      ...
      case Some(h@Head(hp, involved, evalSet)) => {
        ...
        if(cached == None && !(hp::involved contains p)) {
          ...
          return Some(MemoEntry(Right(Failure(in, Set("dummy ")))))
        }
    ...
    case LR(seed ,rule, Some(head)) =>
      if(head.getHead != p) /*not head rule, so not growing*/ seed.asInstanceOf[ParseResult[T]]
        ...
        seed match {
          ...
          case s@Success(_,_,_,_) => /*growing*/ grow(p, in, head)
        }
      }
    ...
    new PackratParser[T] {
      def apply(in: Input) = {
        ...
        m match {
          ...
          case None =>
            val base = LR(Failure(in, Set("Base Failure")), p, None)
