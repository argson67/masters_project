...
trait TreePrinters extends PrettyPrinter {
  def prettyPrintTree(tree: Tree): String = pretty(printTree(tree), 120)

  private[TreePrinters] def vsep2(ds: Seq[Doc]): Doc =
    ssep(ds, line <> line)

  private[TreePrinters] def printTree(tree: Tree): Doc = {
